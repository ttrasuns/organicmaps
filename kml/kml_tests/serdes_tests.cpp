...
UNIT_TEST(Kml_Serialization_Text_File_Wrong_Timestamps_Count)
{
  classificator::Load();
  ...
  auto & brokenTrackData = data.m_tracksData[0];
  brokenTrackData.m_geometry.m_timestamps.clear();
  brokenTrackData.m_geometry.AddTimestamps({1, 2});
  TEST_ANY_THROW(
  {
    kml::SerializerKml ser(data);
    FileWriter sink(kmlFile);
